    ...
    public void loop() {
        ...
        if (!trans) {
            if (gamepad2.left_stick_y < 0 && intakeExtendPosLeft > EXTEND_LEFT_OUT && intakeExtendPosRight < EXTEND_RIGHT_OUT) {
                intakeExtendPosRight += 0.001;
                intakeExtendPosLeft -= 0.001;
...
//                robot.intake.setExtendPosition(EXTEND_LEFT_TRANS, EXTEND_RIGHT_TRANS);
//                robot.intake.setLeftRightPosition(0.5, 0.5);
...
//         Scoring Control
        if (gamepad2.y && !yHold && !locked) {
            robot.intake.setExtendPosition(EXTEND_RIGHT_TRANS_PREP, EXTEND_LEFT_TRANS_PREP);
