...
@TeleOp
public class A_Competition_D extends OpMode {
    private Telemetry telemetryA;
    RobotConstants.AllianceColour color = RobotConstants.AllianceColour.Both;
    ...
    private boolean manual = false, basket = false, back = true, armDown = false, trans = false, ledOn = true, fieldOriented = false;
    ...
    public void loop() {
        ...
        x = -(Math.abs(gamepad1.left_stick_x) > 0.2 ? gamepad1.left_stick_x : 0) * 0.9;
        rx = -gamepad1.right_stick_x * 0.85;
        if (gamepad1.back && !prev.back) {
            fieldOriented = !fieldOriented;
        }
        if (gamepad1.start) robot.drivetrain.pinPoint.resetPosAndIMU();
        if (fieldOriented) {
        ...
        if (gamepad1.dpad_up && !prev.dpad_up) {
            ...
            extendPercent += 50;
            if (extendPercent == 150) extendPercent = 50;
        ...
        if (gamepad1.right_trigger > 0 && robot.scoring.getLiftPosition() >= 0) {
            robot.scoring.setLiftPower(-1);
            manual = true;
        } else if (gamepad1.left_trigger > 0 && robot.scoring.getLiftPosition() <= LIFT_HIGH_BASKET) {
            robot.scoring.setLiftPower(1);
            manual = true;
        } else if (manual) {
